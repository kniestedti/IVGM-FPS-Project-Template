...
public class HSProjectileStandard : MonoBehaviour
{
    [Header("General")] 
    ...
    void OnHit(Vector3 point, Vector3 normal, Collider collider)
    {
        ...
        if(knockbackRadius <= 0){
            ...
            if(rb != null){
                ...
                //Debug.Log("Force added: " + direction.x + " " + direction.y + " " + direction.z);
            }
        } else {
            Collider[] collides = Physics.OverlapSphere(transform.position, knockbackRadius);

            for(int i = 0; i < collides.Length; i++){
                Rigidbody rb = collides[i].GetComponent<Rigidbody>();
                if(rb != null){

                    rb.AddExplosionForce(knockbackStrength, transform.position, knockbackRadius);
                    //Debug.Log("EXPLOSION!!!! at" + transform.position.x + " " + transform.position.y + " " + transform.position.z);
                }
            }
        }
